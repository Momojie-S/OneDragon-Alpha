...
**预期结果**:
> **注意**: localStorage 在同一域名下的所有标签页之间共享，因此实际的预期行为如下：

- [x] 标签页 B 的选择会覆盖 localStorage 中的值
- [x] 刷新标签页 A 后会读取到标签页 B 最后保存的选择 Y
- [x] 这是 localStorage 的正常行为，符合设计预期

> **如需真正的标签页独立**，应该使用 `sessionStorage` 而非 `localStorage`，但本功能设计为跨标签页共享选择，因此当前行为是正确的。
