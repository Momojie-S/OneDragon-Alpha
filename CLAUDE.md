...
1. 当启动一个新的需求修改时，需要使用 opsx 草拟方案，并在后续过程按 opsx 规范进行。
2. 在设计阶段，你应该:
...
4. 完成后端开发后，再进行前端开发：
  4.1. 前端代码开发。
  4.2. 制定前端代码的端到端测试计划，但暂不需要实现测试代码，仅做少量的核心场景。
  4.3. 开发测试数据清理接口（如测试会产生脏数据）。
  4.4. 使用 chrome-devtools 按前端的端到端测试计划进行测试，过程中按发现问题调整代码或者调整测试计划。
